...
public class TransactionManager {
  ...
  public int getTotalTransactions() { return this.totalTransactions; }

  public int getTotalSoldItems() { return this.totalSoldItems; }

  public double getTotalProfit() { return this.totalProfit; }

  public void registerTransaction(String itemID, int amountOfItem,
                                  double totalPrice, String itemInfo) {
    Transaction newTransaction =
        new Transaction(itemID, amountOfItem, totalPrice, itemInfo);
  ...
  // Concatenate printing information such as printing message, item info and
  ...
  public String printItemTransaction(String itemID) {
    ...
    if (totalNumberOfTransaction(itemID) == 0) {
      printTransaction = printTransaction + itemInfo + s +
                         "No transactions have been registered for item " +
                         itemID + " yet";
    } else {
  ...
  public String printAllTransactions() {
    ...
    String printTransactions =
        "All purchases made:" + s + "Total profit: " + totalProfit + " SEK" +
        s + "Total items sold: " + totalSoldItems + " units" + s +
  ...
  public String getMostProfitableItem() {
    ...
    for (int i = 0; i < transactionList.size(); i++) {
      Transaction currentTransaction = transactionList.get(i);
      if (currentTransaction.getSumPurchase() <
          transactionList.get(i + 1).getSumPurchase()) {
      }
    }
    return "";
  }
